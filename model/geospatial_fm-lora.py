...
class CustomAttention(Attention):
    ...
    def forward(self, x):
        ...
        # fiq:
        # q = self.lora_q(q).permute(0, 2, 1, 3) : check this line
...
class TemporalViTEncoder(nn.Module):
    ...
    def __init__(
        ...
        if lora_config is not None:
            ...
            self.lora_rank = lora_config.get('rank', 8)
            self.adapter_size = lora_config.get('adapter_size', 768)
        ...
        self.blocks = nn.ModuleList(
            ...
            CustomBlock(
                ...
                qkv_bias=True, 
                norm_layer=norm_layer, 
                lora_adapter_config=lora_config)
            for _ in range(depth)
        ])
    ...
    def initialize_weights(self):
        if isinstance(self.pretrained, str):
            ...
            # Extract the state_dict from checkpoint, if present
            state_dict = checkpoint.get('state_dict', checkpoint)
            
            # Filter out unnecessary keys
            model_state_dict = self.state_dict()
            checkpoint_state_dict = {k: v for k, v in state_dict.items() if k in model_state_dict and model_state_dict[k].shape == v.shape}
    
            # Load the filtered state dict
            self.load_state_dict(checkpoint_state_dict, strict=False)
    
            # Optionally, initialize missing weights specifically for LoRA adapters
            if any("lora" in key for key in model_state_dict.keys()):
                self.initialize_lora_adapters()
    
            print(f"Loaded pretrained weights from {self.pretrained}, without LoRA parameters.")
        else:
            # Apply default initializations
